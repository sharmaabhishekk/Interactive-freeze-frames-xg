...
var InteractiveSVG = (function() {
    ...
    **************************************************/
    ...
    InteractiveSVG.prototype._addMouseEventHandlers = function() {
        ...
        this.$svg.on('pointermove', function(evt) {
            if (self.selected) {
                // Get dragging to work on touch device
                if (evt.type === 'touchmove') { evt = evt.touches[0]; console.log("touchmove found")}
        ...
        this.$svg.on('pointerup', function() {
            self.selected = false;
        });
    };
    ...
    InteractiveSVG.prototype._setAsDraggable = function(element) {
        var self = this;
        element.$element.on('pointerdown', function(evt) {
            self.selected = element;

            // Get dragging to work on touch device
            if (evt.type === 'touchstart') { evt = evt.touches[0]; console.log("touch start found")}
            self.dragX = evt.clientX;
            self.dragY = evt.clientY;
        });
    };
